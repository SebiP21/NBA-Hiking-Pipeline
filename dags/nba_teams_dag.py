...
def nba_teams_pipeline():
    ...
    # initializing the results table in postgres
    ...
    def compute_and_load_teams():
        ...
        # extracting numbers from the trail descriptions
        ...
        # creating trail requirements based on distance and danger
        ...
        # trimming down to just the columns we need before the join
        ...
        # cross joining everyone against every trail
        pairs = p_mini.merge(t_mini, on='_k').drop(columns=['_k'])

        # applying the compatibility rules
        ...
        # first getting the count per player
        player_counts = (compatible.groupby(['NAME', 'TEAM'])['Trail Name']
                         .nunique()
                         .reset_index(name='player_trail_count'))

        # now averaging those counts by team to find the best overall team
        ...
        # pushing the team stats to postgres
